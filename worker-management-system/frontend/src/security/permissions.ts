...
export type PermissionCode =
  ...
  | 'MASTER_VIEW' | 'MASTER_EDIT' | 'FORMULA_VIEW' | 'FORMULA_EDIT' | 'GOVERNANCE_VIEW'
...
const all: PermissionCode[] = ['DASHBOARD_VIEW','REPORT_PENDING_VIEW','REPORT_APPROVE','REPORT_PENDING_EDIT','REPORT_APPROVED_VIEW','REPORT_APPROVED_EDIT','REPORT_DELETE','REPORT_EXPORT','EXCEL_DB_SYNC','EXCEL_MASTER_SYNC','USER_VIEW','USER_CREATE','USER_EDIT','MASTER_VIEW','MASTER_EDIT','FORMULA_VIEW','FORMULA_EDIT','GOVERNANCE_VIEW','PERIOD_LOCK','PERIOD_UNLOCK','STATISTICS_VIEW','NOTIFICATION_VIEW','AUDIT_VIEW','SYSTEM_HEALTH_VIEW','PERMISSION_MANAGE','WORKER_ENTRY','WORKER_HISTORY','PROFILE_VIEW'];
...
export function defaultPermissionsForRole(role?: string): Set<PermissionCode> {
  return new Set(defaults[String(role||'').toLowerCase()] || []);
}

export function clearPermissionClientCache(){
  cache=null;
}
...
export async function loadMyPermissions(force=false): Promise<Set<PermissionCode>> {
  ...
  if(!force && cache && cache.userId===userId && cache.expiresAt>Date.now()) {
    return new Set(cache.values);
  }

  // Multiple layouts/routes can mount usePermissions at the same time.
  // Share the same in-flight request instead of creating a request storm.
  if(inFlight?.userId===userId) {
    return new Set(await inFlight.promise);
  }
