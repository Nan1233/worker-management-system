...
const CANONICAL_SCHEMA_PATH = path.join(
  __dirname,
  '../database/KTC_FULL_DATABASE_CANONICAL_20260817.sql',
);
...
function findCreateTableBodies(sql) {
  ...
  while ((match = header.exec(sql))) {
    ...
    if (closeIndex < 0) {
      throw new Error(`Canonical SQL parse error: unterminated CREATE TABLE ${table}`);
    }
...
function parseColumnDefinition(definition) {
  ...
  if (/^(PRIMARY|UNIQUE|KEY|CONSTRAINT|FULLTEXT|SPATIAL)\b/i.test(text)) {
    return null;
  }
  ...
  const extra = [
    ...
    /\bON\s+UPDATE\s+CURRENT_TIMESTAMP(?:\(\d+\))?\b/i.test(rest)
  ...
  return {
    ...
    type,
    nullable,
    default: normalizeDefault(defaultMatch ? defaultMatch[1] : null),
    extra,
...
function parseIndexDefinition(definition) {
  ...
  if (match) {
    return {
      name: 'primary',
      unique: true,
      columns: splitTopLevel(match[1]).map((s) => s.trim().replace(/^`|`$/g, '').toLowerCase()),
    };
  }
...
function loadCanonicalSchema() {
  ...
  return Object.freeze({
    ...
    source: path.relative(process.cwd(), CANONICAL_SCHEMA_PATH),
  });
}
...
function compareColumn(expected, actual) {
  ...
  if (String(actual.COLUMN_TYPE || '').toLowerCase() !== expected.type) {
    diffs.push(`type=${actual.COLUMN_TYPE || 'NULL'} expected=${expected.type}`);
  }

  const nullable = String(actual.IS_NULLABLE || '').toUpperCase() === 'YES';
  if (nullable !== expected.nullable) {
    diffs.push(`nullable=${nullable} expected=${expected.nullable}`);
  }

  const actualDefault = normalizeDefault(actual.COLUMN_DEFAULT);
  if (actualDefault !== expected.default) {
    diffs.push(`default=${actualDefault ?? 'NULL'} expected=${expected.default ?? 'NULL'}`);
  }

  const actualExtra = String(actual.EXTRA || '').toLowerCase();
  const expectedAuto = expected.extra.includes('auto_increment');
  const actualAuto = actualExtra.includes('auto_increment');

  if (expectedAuto !== actualAuto) {
    diffs.push(`auto_increment=${actualAuto} expected=${expectedAuto}`);
  }

  const expectedUpdate = expected.extra.includes('on_update_current_timestamp');
  const actualUpdate = actualExtra.includes('on update current_timestamp');

  if (expectedUpdate !== actualUpdate) {
    diffs.push(`on_update_current_timestamp=${actualUpdate} expected=${expectedUpdate}`);
  }
...
function compareIndex(expected, rows) {
  ...
  if (unique !== expected.unique) {
    diffs.push(`unique=${unique} expected=${expected.unique}`);
  }

  if (JSON.stringify(columns) !== JSON.stringify(expected.columns)) {
    diffs.push(`columns=${columns.join(',')} expected=${expected.columns.join(',')}`);
  }
