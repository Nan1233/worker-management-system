...
test('legacy cutover helper now enforces full Part 2 context',()=>assert.throws(()=>assertCutoverAllowed({finalState:'VERIFIED_NOT_ACTIVATED',schemaReady:true,schemaStatus:'READY',integrityReady:true,sessionsInvalidated:true,activeSessionsRemaining:0,restoreId:'restore_test',backupSha256:'a'.repeat(64)}),e=>e.code==='CUTOVER_BLOCKED'));
...
test('restore state must pass canonical schema verification before activation',()=>{const s=read('scripts/disasterRestoreDatabase.js');const schema=s.indexOf("'verifyDatabaseSchema.js'");const final=s.indexOf("VERIFIED_NOT_ACTIVATED");assert.ok(schema>0&&final>schema);});
