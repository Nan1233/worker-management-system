...
test('logout flow uses canonical server logout while clearing local identity immediately', () => {
  ...
  const worker = read('frontend/src/layouts/WorkerLayout.tsx');
  const manager = read('frontend/src/layouts/ManagementLayout.tsx');
...
test('manager pending flow keeps admin navigation inside the admin route family', () => {
  const reports = read('frontend/src/pages/manager/Reports.tsx');
