...
const css = fs.readFileSync(path.join(root, "frontend/src/components/feedback/toast.css"), "utf8");
const provider = fs.readFileSync(path.join(root, "frontend/src/components/feedback/ToastProvider.tsx"), "utf8");
const sw = fs.readFileSync(path.join(root, "frontend/public/sw.js"), "utf8");
const version = fs.readFileSync(path.join(root, "frontend/src/config/version.ts"), "utf8");
...
assert.doesNotMatch(css, /(?<!ktc-)\.toast-container\s*\{/);
