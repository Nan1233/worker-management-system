...
const wrappedServer = { async fetch(request, envArg, ctx) { const preflight = handleCorsPreflight(request); if (preflight) return preflight; const pathname = new URL(request.url).pathname; if (pathname === "/api/health" || pathname === "/api/health/ready") { if (!runtimeReadiness.ready && !runtimeReadiness.initializing) { void initializeRuntime(); } } else if (shouldBootstrapBeforeRequest(request)) { const seeded = await ensureCloudflareSeeded(); if (!seeded) console.warn("[KTC] Continuing request without master-data bootstrap; seed will retry on a later request."); } const response = await httpHandler.fetch(request, envArg, ctx); return enrichApprovedReportMachineDefects(request, response); } };
