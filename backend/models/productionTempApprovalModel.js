...
const { query, getConnection, beginTransaction, commit, rollback, normalizeIds } = require("./productionTempModelShared");
...
function assertReviewBatchSize(ids) {
  if (!Array.isArray(ids) || ids.length === 0) throw new Error("Danh sách báo cáo không hợp lệ");
  if (ids.length > 100) throw new Error("Chỉ được duyệt tối đa 100 báo cáo mỗi lần");
}

function datesForExcel(rows) {
  return [...new Set(rows.map((row) => String(row.work_date || "").slice(0, 10)).filter(Boolean))];
}
...
module.exports = {
  async approveSelected(targets, reviewerId, options = {}) {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (!["pending", "need_fix"].includes(String(row.status))) {
          throw new Error(`Báo cáo #${row.id} không ở trạng thái có thể duyệt`);
        }
        ...
        const workDate = row.work_date instanceof Date
          ? `${row.work_date.getFullYear()}-${String(row.work_date.getMonth()+1).padStart(2,"0")}-${String(row.work_date.getDate()).padStart(2,"0")}`
      ...
      for (const notification of postCommitNotifications) {
        ...
        catch (error) { console.warn(`[KTC] Post-commit approval notification failed: ${error.message}`); }
      }
  ...
  async rejectSelected(targets, reviewerId, reason, isAdmin = false) {
    ...
    try {
      ...
      const [rows]=await query(connection,`SELECT DISTINCT temp.id,temp.worker_id,temp.process_id,temp.work_date,temp.shift,temp.updated_at,w.user_id AS worker_user_id FROM production_reports_temp temp JOIN workers w ON w.id=temp.worker_id ${scopeJoin} WHERE temp.id IN (${placeholders}) AND temp.status IN ('pending','need_fix') ${scopeWhere} FOR UPDATE`,params);
      if(rows.length!==reportIds.length) throw new Error('Có báo cáo không tồn tại, đã xử lý hoặc người phê duyệt không có quyền truy cập');
      for(const row of rows){
        const expected=expectedById.get(Number(row.id));
        if(expected&&new Date(expected).getTime()!==new Date(row.updated_at).getTime()){
          const error=new Error(`Báo cáo #${row.id} đã thay đổi sau khi bạn mở danh sách. Hãy tải lại trước khi thử lại.`);
      ...
      await commit(connection);
      for(const notification of postCommitNotifications){try{await AuditService.notifyUsers(notification.userIds,notification.payload);}catch(error){console.warn(`[KTC] Post-commit rejection notification failed: ${error.message}`);}}
