...
module.exports = {
  ...
  async approveSelected(ids, reviewerId, options = {}) {
    ...
    try {
      ...
      if (!reportIds.length) {
        const err = new Error('Không có báo cáo hợp lệ để duyệt');
        err.status = 400;
        throw err;
      }
      ...
      const [rows] = await query(
        connection,
        `SELECT * FROM production_reports_temp WHERE id IN (${placeholders}) FOR UPDATE`,
        reportIds,
      );
      ...
      for (const reportId of reportIds) {
        ...
        if (!temp) continue;
        if (String(temp.status) !== 'pending') continue;
      ...
      try { await rollback(connection); } catch (_) {}
      throw error;
    } finally {
      connection.release();
    }
