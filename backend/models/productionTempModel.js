...
/**
 * Enforce the 12-hour daily limit.
 *
 * IMPORTANT for Cloudflare + TiDB Serverless:
 * do not use GET_LOCK/RELEASE_LOCK here. GET_LOCK is session-scoped, while
 * the serverless driver is designed around stateless HTTP queries. Keeping a
 * user-level advisory lock open across the whole report-creation flow can
 * therefore leave the request waiting until the 10s timeout and return 503.
 *
 * Concurrency/idempotency is already protected by the transaction-level
 * duplicate locks inside productionTempCreateModel. This check intentionally
 * remains a normal stateless read so a valid report is not blocked by a
 * session-scoped lock that cannot be safely held in Cloudflare Workers.
 */
...
/**
 * Kept as a compatibility no-op for callers that still pass the previous
 * lock state. There is intentionally no session-level lock to release.
 */
const releaseDailyWorkerHoursLock = async () => {};
 ...
 * so training snapshots, duplicate protection and report creation all use
 * the same authenticated worker identity.
 */
const createCompleteReport = async (payload = {}, legacyDefects, legacyDeductions, legacyMachineLines, legacyAudit) => {
    ...
    // The daily-hours check is deliberately stateless for Cloudflare/TiDB
    // Serverless. Do not keep a session-level advisory lock open while the
    // report transaction creates defects, deductions, machine lines and audit.
