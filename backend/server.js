...
app.use((req, res, next) => {
  ...
  const requestId = /^[A-Za-z0-9._:-]{1,120}$/.test(rawRequestId) ? rawRequestId : crypto.randomUUID();
  ...
  res.on("finish", () => {
    ...
    runtimeMetrics.recordHttp({ requestId, method: req.method, path: requestPath, status: res.statusCode, durationMs });
...
// Cloudflare/esbuild can expose a CommonJS route as { default: router }.
// Normalize every route before Express sees it and fail with the exact module name.
app.use("/api/mobile", resolveRouteModule(mobileRoutes, "mobileRoutes"));
app.use("/api/network", resolveRouteModule(networkAccessRoutes, "networkAccessRoutes"));
app.use("/api/dashboard", resolveRouteModule(dashboardRoutes, "dashboardRoutes"));
app.use("/api/machine-production-events", resolveRouteModule(machineProductionEventRoutes, "machineProductionEventRoutes"));
app.use("/api/machines", resolveRouteModule(machineRoutes, "machineRoutes"));
app.use("/api/product-standards", resolveRouteModule(productStandardRoutes, "productStandardRoutes"));
app.use("/api/auth", resolveRouteModule(authRoutes, "authRoutes"));
app.use("/api/users", resolveRouteModule(userRoutes, "userRoutes"));
app.use("/api/workers", resolveRouteModule(workerRoutes, "workerRoutes"));
app.use("/api/production", resolveRouteModule(productionRoutes, "productionRoutes"));
app.use("/api/production-temp", resolveRouteModule(productionTempRoutes, "productionTempRoutes"));
app.use("/api/manager", resolveRouteModule(managerRoutes, "managerRoutes"));
app.use("/api/reports", resolveRouteModule(reportExportRoutes, "reportExportRoutes"));
app.use("/api/sync-jobs", resolveRouteModule(syncJobRoutes, "syncJobRoutes"));
app.use("/api/version", resolveRouteModule(require("./routes/versionRoutes"), "versionRoutes"));
app.use("/api/system", resolveRouteModule(systemRoutes, "systemRoutes"));
app.use("/api/permissions", resolveRouteModule(permissionRoutes, "permissionRoutes"));
app.use("/api/admin/master", resolveRouteModule(adminMasterRoutes, "adminMasterRoutes"));
app.use("/api/formula-settings", resolveRouteModule(formulaSettingsRoutes, "formulaSettingsRoutes"));
app.use("/api/governance", resolveRouteModule(governanceRoutes, "governanceRoutes"));
app.use("/api/excel-master-sync", resolveRouteModule(excelMasterSyncRoutes, "excelMasterSyncRoutes"));
app.use("/api", resolveRouteModule(defectRoutes, "defectRoutes"));
app.use("/api", resolveRouteModule(deductionRoutes, "deductionRoutes"));
...
async function warmFrequentlyUsedMasterData() {
  try {
    const [processRows] = await db.promise().query("SELECT id FROM processes WHERE status = 'active' ORDER BY id LIMIT 20");
  ...
  } catch (error) { console.warn(`Master data warmup skipped: ${error.message}`); }
}
...
function startDatabaseKeepAlive() {
  ...
  databaseKeepAliveTimer = setInterval(() => {
    void db.promise().query({ sql: "SELECT 1 AS ok", timeout: 5_000 }).catch((error) => console.warn(`Database keepalive failed: ${error.message}`));
  }, intervalMs);
...
async function initializeRuntime() {
  ...
  try {
    ...
    if (startupFailureTimer) { clearTimeout(startupFailureTimer); startupFailureTimer = null; }
    ...
    runtimeReadiness.initializing = false;
    runtimeReadiness.database = error?.schemaStatus === "DATABASE_UNAVAILABLE" ? "unavailable" : runtimeReadiness.database;
    ...
    console.error(JSON.stringify({ type: "startup_not_ready", code: runtimeReadiness.errorCode, schemaStatus: runtimeReadiness.schemaStatus, schemaContractVersion: runtimeReadiness.schemaContractVersion, message: runtimeReadiness.errorMessage }));
    if (!startupFailureTimer) {
      const exitDelayMs = Math.max(5_000, Number(process.env.STARTUP_FAILURE_EXIT_MS || 20_000));
      startupFailureTimer = setTimeout(() => { console.error(`[KTC] runtime still not ready after startup failure; exiting in ${exitDelayMs}ms policy`); process.exit(1); }, exitDelayMs);
...
async function start() {
  try { validateEnvironment(process.env, { production: isProduction }); }
  catch (error) { console.error(JSON.stringify({ type: "startup_environment_invalid", code: error?.code || "ENVIRONMENT_INVALID", message: error?.message })); process.exitCode = 1; return null; }
...
async function shutdown(signal) {
  ...
  if (server) {
    await Promise.race([
      ...
      new Promise((resolve) => setTimeout(() => { server.closeAllConnections?.(); resolve(); }, 10_000)),
    ]);
  }
