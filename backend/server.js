...
function parseCorsOrigins() {
  ...
  return new Set(["http://localhost:5173", "https://localhost", "capacitor://localhost", "https://worker-management-system-3-dzox.onrender.com", ...configured]);
}
...
mount("/api/product-standards", productStandardRoutes, "productStandardRoutes");
mount("/api/auth", authRoutes, "authRoutes");
mount("/api/users", userRoutes, "userRoutes");
mount("/api/workers", workerRoutes, "workerRoutes");
mount("/api/production", productionRoutes, "productionRoutes");
mount("/api/production-temp", productionTempRoutes, "productionTempRoutes");
mount("/api/manager", managerRoutes, "managerRoutes");
mount("/api/reports", reportExportRoutes, "reportExportRoutes");
mount("/api/sync-jobs", syncJobRoutes, "syncJobRoutes");
mount("/api/version", require("./routes/versionRoutes"), "versionRoutes");
mount("/api/system", systemRoutes, "systemRoutes");
mount("/api/permissions", permissionRoutes, "permissionRoutes");
mount("/api/admin/master", adminMasterRoutes, "adminMasterRoutes");
mount("/api/formula-settings", formulaSettingsRoutes, "formulaSettingsRoutes");
mount("/api/governance", governanceRoutes, "governanceRoutes");
mount("/api/excel-master-sync", excelMasterSyncRoutes, "excelMasterSyncRoutes");
...
async function initializeRuntime() { runtimeReadiness.initializing = true; runtimeReadiness.startedAt = Date.now(); runtimeReadiness.errorCode = null; runtimeReadiness.errorMessage = null; try { const databaseStartedAt = Date.now(); const database = await db.testConnection(); runtimeReadiness.databaseLatencyMs = Date.now() - databaseStartedAt; runtimeReadiness.database = "ok"; console.log(`[KTC] Database connected; SSL=${database.ssl}`); const schema = await assertDatabaseSchemaReady(); const diagnostics = toSafeSchemaDiagnostics(schema); runtimeReadiness.schemaReady = true; runtimeReadiness.schemaStatus = diagnostics.status; runtimeReadiness.schemaContractVersion = diagnostics.contractVersion; console.log(`Database schema READY: ${schema.expectedLatest?.filename || "none"}`); await excelExportJobQueue.initialize(); runtimeReadiness.ready = true; runtimeReadiness.readyAt = Date.now(); runtimeReadiness.initializing = false; if (startupFailureTimer) { clearTimeout(startupFailureTimer); startupFailureTimer = null; } console.log(`[KTC] runtime READY in ${runtimeReadiness.readyAt - runtimeReadiness.startedAt}ms`); startDatabaseKeepAlive(); void warmFrequentlyUsedMasterData(); void logProductionIndexAudit(db); startProductionIndexAuditScheduler(db); } catch (error) { const safeDetails = error?.details || {}; runtimeReadiness.ready = false; runtimeReadiness.initializing = false; runtimeReadiness.database = error?.schemaStatus === "DATABASE_UNAVAILABLE" ? "unavailable" : runtimeReadiness.database; runtimeReadiness.schemaStatus = error?.schemaStatus || "STARTUP_FAILED"; runtimeReadiness.schemaContractVersion = safeDetails.contractVersion || 26; runtimeReadiness.errorCode = error?.code || "DATABASE_STARTUP_FAILED"; runtimeReadiness.errorMessage = error?.message || "Runtime initialization failed"; console.error(JSON.stringify({ type: "startup_not_ready", code: runtimeReadiness.errorCode, schemaStatus: runtimeReadiness.schemaStatus, schemaContractVersion: runtimeReadiness.schemaContractVersion, message: runtimeReadiness.errorMessage })); if (!startupFailureTimer) { const exitDelayMs = Math.max(5000, Number(process.env.STARTUP_FAILURE_EXIT_MS || 20000)); startupFailureTimer = setTimeout(() => { console.error(`[KTC] runtime still not ready after startup failure; exiting in ${exitDelayMs}ms policy`); process.exit(1); }, exitDelayMs); startupFailureTimer.unref?.(); } } }
async function start() { if (!isCloudflareWorker) { try { validateEnvironment(process.env, { production: isProduction }); } catch (error) { console.error(JSON.stringify({ type: "startup_environment_invalid", code: error?.code || "ENVIRONMENT_INVALID", message: error?.message })); process.exitCode = 1; return null; } } else { console.log("[KTC] Cloudflare Worker mode: skipping Node-only environment validation"); }
  server = app.listen(PORT, "0.0.0.0", () => { const version = require("./config/version"); console.log(`[KTC] Server running; port=${PORT}`); console.log(`[KTC] backendVersion=${version.backendVersion} commitSha=${version.commitSha} apiVersion=${version.apiVersion} schemaVersion=${version.schemaVersion}`); void initializeRuntime(); });
  server.requestTimeout = Math.max(30000, Number(process.env.HTTP_REQUEST_TIMEOUT_MS || 120000)); server.headersTimeout = Math.max(15000, Number(process.env.HTTP_HEADERS_TIMEOUT_MS || 30000)); server.keepAliveTimeout = Math.max(5000, Number(process.env.HTTP_KEEPALIVE_TIMEOUT_MS || 10000)); return server; }
async function shutdown(signal) { console.log(`${signal} received; shutting down`); if (databaseKeepAliveTimer) clearInterval(databaseKeepAliveTimer); if (server) await Promise.race([new Promise(resolve => server.close(resolve)), new Promise(resolve => setTimeout(() => { server.closeAllConnections?.(); resolve(); }, 5000))]); }
if (!isCloudflareWorker) { const serverPromise = start(); process.once("SIGTERM", () => void shutdown("SIGTERM")); process.once("SIGINT", () => void shutdown("SIGINT")); }
module.exports = { app, start, shutdown, initializeRuntime, runtimeReadiness };