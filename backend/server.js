...
async function shutdown(signal) { console.log(`${signal} received; shutting down`); if (databaseKeepAliveTimer) clearInterval(databaseKeepAliveTimer); if (server) await Promise.race([new Promise(resolve => server.close(resolve)), new Promise(resolve => setTimeout(() => { server.closeAllConnections?.(); resolve(); }, 10000))]); await db.closePool().catch(error => console.error("Pool close failed:", error.message)); process.exit(0); }
