...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    // Cloudflare/TiDB driver does not support transaction control reliably; keep the write path statement-level.
    const userInsert = buildInsert('users', { username, password: await bcrypt.hash(password, 10), full_name: fullName, role, status: body.status === 'inactive' ? 'inactive' : 'active' });
    ...
    if (role === 'worker') {
      ...
      if (!Number.isFinite(trainingPercent) || trainingPercent < 0 || trainingPercent > 100) return res.status(400).json({ success: false, message: '% học việc phải từ 0 đến 100' });
      const workerInsert = buildInsert('workers', { user_id: userId, worker_code: String(body.worker_code || '').trim() || null, phone: String(body.phone || '').trim() || null, department: String(body.department || 'Sản xuất').trim() || 'Sản xuất', position: String(body.position || 'Công nhân').trim() || 'Công nhân', training_percent: trainingPercent, status: body.status === 'inactive' ? 'inactive' : 'active' });
