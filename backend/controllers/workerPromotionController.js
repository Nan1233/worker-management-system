...
exports.promoteWorkerToLead = async (req, res) => {
  ...
  try {
    ...
    if (!(await assertManagerScope(req, connection, target.worker_id))) {
      return res.status(403).json({ success:false, message:'Bạn không có quyền nâng công nhân này lên tổ trưởng' });
    }
...
exports.promoteWorkerToManager = async (req, res) => {
  ...
  try {
    ...
    if (String(req.user?.role || '').toLowerCase() !== 'admin') {
      return res.status(403).json({ success:false, message:'Chỉ Admin mới có quyền nâng công nhân lên Quản lý' });
    }
    ...
    await connection.query('UPDATE users SET role=\'manager\', status=\'active\', position=\'Quản lý\' WHERE id=?', [targetId]);
    await connection.query('DELETE FROM worker_processes WHERE worker_id=?', [target.worker_id]);
    for (const assignment of assignments) {
      await connection.query(
        `INSERT INTO manager_processes (manager_id,process_id) VALUES (?,?)
    ...
    clearWorkerProfile(targetId);
    deleteCachedAuthUser(targetId);
    return res.json({ success:true, message:`Đã nâng ${target.full_name || target.username} lên Quản lý`, data:{ user_id:targetId, role:'manager', username:target.username, process_ids:assignments.map(x=>Number(x.process_id)) } });
  } catch (error) {
    try { await connection.rollback(); } catch (_) {}
    if (error?.code === 'ER_DUP_ENTRY') return res.status(409).json({ success:false, message:'Tài khoản đã được phân công công đoạn này' });
