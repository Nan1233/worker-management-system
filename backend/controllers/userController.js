  ...
  try {
    ...
    const [rows] = await db.promise().query(
      `SELECT u.id, u.username, u.full_name, u.role, u.status, u.created_at,
              ...
              GROUP_CONCAT(DISTINCT COALESCE(pw.process_name, pm.process_name) ORDER BY COALESCE(pw.process_name, pm.process_name) SEPARATOR ', ') AS process_names
       FROM users u
...
exports.updateUser = async (req, res) => {
  ...
  try {
    ...
    if (found[0].role === 'worker') {
      for (const field of ['worker_code','phone','department','position','training_percent']) if (Object.prototype.hasOwnProperty.call(req.body || {}, field)) workerPayload[field] = req.body[field] === '' ? null : req.body[field];
...
exports._test = { manageableRoles, normalizeProcessIds, normalizeStatus };
