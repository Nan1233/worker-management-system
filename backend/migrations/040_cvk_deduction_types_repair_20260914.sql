-- KTC 040: Repair/ensure the CVK deduction catalogue.
-- CVK must expose the same canonical 10 deduction types used by Gia công.
-- Idempotent: existing rows are reused; missing rows are inserted.
...
WHERE UPPER(TRIM(p.process_code)) = 'CVK'
  ...
  AND NOT EXISTS (
    ...
    WHERE d.process_id = p.id
      AND (LOWER(TRIM(COALESCE(d.deduction_code, ''))) = LOWER(TRIM(v.deduction_code))
