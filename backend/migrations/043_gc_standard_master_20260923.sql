...
-- Cloudflare/TiDB migration runner executes statements through remote subrequests.
-- Keep this migration free of transaction/session-variable statements and avoid
-- repeated scalar subqueries that multiply remote DB work.
...
-- Resolve GC once per statement via a join. Do not use @session variables.
UPDATE product_standards ps
JOIN processes p ON p.id = ps.process_id
JOIN tmp_gc_standards_20260923 c ON c.product_code = ps.product_code
...
-- Keep one active canonical row. Duplicate historical rows are retained but inactive.
UPDATE product_standards ps
JOIN processes p ON p.id = ps.process_id
JOIN (
...
-- Deactivate active GC standards outside the canonical master.
UPDATE product_standards ps
JOIN processes p ON p.id = ps.process_id
LEFT JOIN tmp_gc_standards_20260923 c ON c.product_code = ps.product_code
SET ps.status = 'inactive'
WHERE UPPER(TRIM(p.process_code)) = 'GC'
  AND ps.status = 'active'
  AND c.product_code IS NULL;

-- Verification statements are intentionally lightweight.
