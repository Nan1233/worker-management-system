...
function loadCanonicalSchema() {
  ...
  const sql = stripSqlComments(fs.readFileSync(schemaPath, 'utf8'));
  const tables = {};
  ...
  return Object.freeze({ tables, version: CONTRACT_VERSION, source: schemaPath });
}
...
function compareColumn(expected, actual) {
  ...
  if (expectedAuto !== actualAuto) diffs.push(`auto_increment=${actualAuto} expected=${expectedAuto}`);
  ...
  if (expectedUpdate !== actualUpdate) diffs.push(`on_update_current_timestamp=${actualUpdate} expected=${expectedUpdate}`);
  return diffs;
}
