...
const MAX_DEFECTS = 15;
const LAST_COLUMN = 53; // BA, following the Cắt lồng reference layout.
...
function clearSheetData(sheet) {
  for (const range of [...sheet.mergedCells || []]) {
    try { sheet.unMergeCells(range); } catch (_) {}
  }
...
function copyTemplateStyles(sheet, sourceRow, targetRow, styles) {
  ...
  row.height = sheet.getRow(sourceRow).height || 18;
...
function setHeader(sheet, rowNumber, styles, deductions, defects) {
  ...
  const headers = [
    'STT', 'Mã CN', 'Họ tên', 'Số máy', 'Ca', '% học việc',
    'Tổng thời gian', 'Thời gian thực tế', 'Số lần CM', 'Tổng TG trừ giờ', 'Thiếu sản lượng',
    ...Array.from({ length: MAX_DEDUCTIONS }, (_, i) => deductions[i]?.name || ''),
    'SP', 'Định mức', 'TT', 'Tỷ lệ đạt', 'Ngày', 'SLSP/h', 'OK', 'Tổng NG', 'Tỷ lệ NG',
    ...Array.from({ length: MAX_DEFECTS }, (_, i) => defects[i]?.name || ''),
    'OK', 'NG'
  ];
...
function writeReportRow(sheet, rowNumber, report, deductionTypes, defectTypes, dataStyles, index) {
  ...
  const actual = num(report.actual_output);
  const ok = num(report.tt_ok);
  const ng = num(report.tt_ng);
  const achievement = standard > 0 ? actual / standard : 0;
  ...
  const values = [
    ...
    report.machine_no ?? '',
    report.shift ?? '',
    report.training_percent ?? report.training_percent_snapshot ?? null,
    totalTime,
    actualTime,
    detailValue(deductions, null, 'Chuyển mã', 'hours'),
    deductionTime,
    Math.max(0, standard - actual),
    ...
    ng,
    ngRate,
    ...defectTypes.slice(0, MAX_DEFECTS).map((t) => detailValue(defects, t.id, t.name, 'quantity')),
    ok,
    ng
  ];
  for (let c = 1; c <= LAST_COLUMN; c += 1) row.getCell(c).value = values[c - 1] ?? null;
  row.getCell(6).numFmt = '0%';
  row.getCell(24).numFmt = '0%';
  row.getCell(25).numFmt = 'dd/mm/yyyy';
  row.getCell(28).numFmt = '0%';
  ...
  row.getCell(24).font = { ...(row.getCell(24).font || {}), bold: true };
  row.getCell(28).font = { ...(row.getCell(28).font || {}), bold: true };
}

function prepareSheet(sheet) {
  const headerStyles = captureRowStyles(sheet, 3);
  const dataStyles = captureRowStyles(sheet, Math.min(6, sheet.rowCount));
  const dateStyles = captureRowStyles(sheet, 1);
  const widths = Array.from({ length: LAST_COLUMN }, (_, i) => sheet.getColumn(i + 1).width);
  clearSheetData(sheet);
  widths.forEach((width, i) => { if (width != null) sheet.getColumn(i + 1).width = width; });
...
async function buildCommonProcessMonthlyWorkbook(reports, yearMonth, options = {}) {
  ...
  for (const report of reportList) {
    ...
    grouped.get(sheetName).push(report);
  }

  const allSheetNames = new Set(TEMPLATE_SHEETS);
  for (const name of grouped.keys()) allSheetNames.add(name);
  for (const name of [...allSheetNames]) {
    let sheet = workbook.getWorksheet(name);
    if (!sheet) sheet = workbook.addWorksheet(safeSheetName(name));
    const styles = prepareSheet(sheet);
    ...
    if (!items.length) {
      ...
      setHeader(sheet, 3, styles.headerStyles, [], []);
      continue;
    }

    const processDeductions = [...new Map(items.flatMap((r) => r.deductions || []).map((x) => [Number(x.deduction_type_id || x.id), x])).values()]
      .sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0) || Number(a.id || a.deduction_type_id || 0) - Number(b.id || b.deduction_type_id || 0));
    const processDefects = [...new Map(items.flatMap((r) => r.defects || []).map((x) => [Number(x.defect_type_id || x.id), x])).values()]
      .sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0) || Number(a.id || a.defect_type_id || 0) - Number(b.id || b.defect_type_id || 0));
    ...
    for (const report of items) {
      ...
      if (day !== currentDate) {
        if (currentDate !== null) rowNumber += 1; // one blank row between dates
        writeDateRow(sheet, rowNumber, displayDate(report.work_date), styles.dateStyles);
        rowNumber += 1;
        setHeader(sheet, rowNumber, styles.headerStyles, processDeductions, processDefects);
        rowNumber += 1;
        currentDate = day;
      }
      writeReportRow(sheet, rowNumber, report, processDeductions, processDefects, styles.dataStyles, rowNumber);
      rowNumber += 1;
    }
    sheet.views = [{ state: 'frozen', ySplit: 2 }];
    sheet.pageSetup = { ...(sheet.pageSetup || {}), orientation: 'landscape', fitToPage: true, fitToWidth: 1, fitToHeight: 0 };
    sheet.properties = { ...(sheet.properties || {}), defaultRowHeight: 18 };
  }

  workbook.calculation = { fullCalcOnLoad: true, forceFullCalc: true, calcMode: 'auto' };
  const target = getMonthlyTarget(yearMonth, { stageFolder: 'BC công đoạn' });
  await fs.mkdir(target.folder, { recursive: true });
  const tempPath = `${target.filePath}.${process.pid}.${Date.now()}.tmp`;
  await workbook.xlsx.writeFile(tempPath);
  await fs.rename(tempPath, target.filePath);
  await fs.writeFile(target.metadataPath, JSON.stringify({
    version: 1,
    template: path.basename(templatePath),
    layout: 'common-process-daily-blocks',
    yearMonth,
