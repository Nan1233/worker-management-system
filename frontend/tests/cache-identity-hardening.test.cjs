...
test('session cache is namespaced by authenticated identity and session id', () => {
  const cache = read('src/services/sessionCache.ts');
  const auth = read('src/utils/authStorage.ts');
...
test('worker profile response is rejected if it belongs to another authenticated worker', () => {
  const source = read('src/services/workerService.ts');
...
test('master data mutation advances browser-wide cache epoch', () => {
  const cache = read('src/services/masterDataCache.ts');
  const page = read('src/pages/admin/MasterData.tsx');
