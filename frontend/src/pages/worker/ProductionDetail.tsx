      ...
      <main className="detail-container">
                ...
                return (
                  <div key={line.id || index} style={{ padding: 10, border: "1px solid #d9e2ef", borderRadius: 10, background: "#fff" }}>
                    ...
                    {lineDefects.length > 0 && (
                      <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid #edf1f7" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 6 }}>
                          {lineDefects.map((item, defectIndex) => (
                            <div key={item.id || defectIndex} style={{ display: "flex", justifyContent: "space-between", gap: 8, padding: "6px 8px", border: "1px solid #e5eaf1", borderRadius: 8 }}>
                              <span>{item.defect_name || item.defect_code || "NG"}</span><strong>{quantity(item.quantity)}</strong>
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        {defects.length > 0 && (
          <section className="detail-section">
            <h2>Lỗi NG</h2>
            <div className="detail-grid">
              {defects.map((item, index) => <div key={item.id || index}><span>{item.defect_name || item.defect_code || "NG"}</span><strong>{quantity(item.quantity)}</strong></div>)}
            </div>
          </section>
        )}
        ...
        {report.note && (
          <section className="detail-section">
            <h2>Ghi chú</h2>
            <p style={{ whiteSpace: "pre-wrap", margin: 0 }}>{report.note}</p>
          </section>
        )}
      </main>
    </div>
