...
const getMachineCode = (value: string) => String(value || "").trim().toUpperCase().replace(/^MÁY\s*/i, "");
...
function WorkerReportEdit() {
  ...
  const [cutExecutionMode, setCutExecutionMode] = useState<"AUTO" | "NON_AUTO">("AUTO");
  ...
  const machineAutocompleteOptions = useMemo<AutocompleteOption[]>(() => machineOptions.map((machine: any) => ({
    value: String(machine.machine_code || "").trim(),
    label: String(machine.machine_code || "").trim(),
  })).filter((option) => option.value), [machineOptions]);

  const productAutocompleteOptions = useMemo<AutocompleteOption[]>(() => productOptions.map((product) => ({
  ...
  const getMachineAutocompleteOptions = (machineCode: string) => {
    ...
    return machineAutocompleteOptions.filter((option) => {
      ...
      return cutExecutionMode === "AUTO"
        ? GC_AUTOMATIC_MACHINE_CODES.has(code)
        : !GC_AUTOMATIC_MACHINE_CODES.has(code);
    });
  };

  const getProductAutocompleteOptions = (machineCode: string) => {
    if (!machineCode.trim()) return productAutocompleteOptions;
    const filtered = filterProductsForSelection({
      products: productOptions,
      mode: "MACHINE",
      machineCode,
      machineOptions,
      useEncodedMachineSuffix: capabilities.processCode === "GC",
    });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const firstMachineCode = String(data.machine_no || "").split(",")[0].trim();
        setForm({
          ...initialForm,
          ...
          trainingPercent: decimal(data.training_percent), machineNo: firstMachineCode,
        ...
        setCutExecutionMode(GC_AUTOMATIC_MACHINE_CODES.has(getMachineCode(firstMachineCode)) ? "AUTO" : "NON_AUTO");
  ...
  return <div className="ktc-page"><main className="detail-container" style={{ maxWidth: 980 }}>
    ...
    <section className="detail-section"><h2>Thông tin báo cáo</h2><div className="detail-grid">
      ...
      <label><span>Ca</span><select value={form.shift} onChange={(e) => updateForm("shift", e.target.value)}><option>A</option><option>B</option><option>C</option><option>D</option></select></label>
    ...
    {operationMode === "MANUAL" && <section className="detail-section"><h2>Sản phẩm & sản lượng</h2><div className="detail-grid">
      <AutocompleteInput id="productName" label="Mã sản phẩm" value={form.productName} options={productAutocompleteOptions} placeholder={loadingMasterData ? "Đang tải danh mục sản phẩm…" : "Nhập hoặc chọn mã sản phẩm"} required disabled={loadingMasterData} emptyMessage="Không có mã sản phẩm phù hợp" onChange={(value) => updateForm("productName", value)} onSelect={(option) => updateForm("productName", option.value)} />
      <AutocompleteInput id="machineNo" label="Mã máy" value={form.machineNo} options={machineAutocompleteOptions} placeholder={loadingMasterData ? "Đang tải danh mục máy…" : "Nhập hoặc chọn mã máy"} disabled={loadingMasterData} emptyMessage="Không có mã máy phù hợp" onChange={(value) => updateForm("machineNo", value)} onSelect={(option) => updateForm("machineNo", option.value)} selectOnly={false} />
