...
export default function ProcessQualitySection({
    ...
    useEffect(() => {
        ...
        const hiddenSelected = selectedNg.filter((key) => !visibleKeys.has(key));
        hiddenSelected.forEach((key) => onToggleNg(key, false));
        ...
        <section className="worker-form-card worker-quality-section">
                ...
                {showNg && (
                    <div id="worker-ng-options" className="worker-dropdown-options">
                        {visibleNgOptions.length === 0 ? (
                            <div className="worker-dropdown-empty" role="status">Chưa có loại lỗi NG được cấu hình cho công đoạn này.</div>
                        ) : visibleNgOptions.map((item) => (
                            <label key={item.key} className="worker-dropdown-option">
                                <input type="checkbox" className="worker-ng-checkbox" style={{ width: 16, height: 16, minWidth: 16, maxWidth: 16, minHeight: 16, maxHeight: 16, flex: "0 0 16px", boxSizing: "border-box", margin: 0, padding: 0 }} checked={selectedNg.includes(item.key)} onChange={(event) => { if (!qualityLocked) onToggleNg(item.key, event.target.checked); }} disabled={qualityLocked} />
            ...
            {selectedNg.length > 0 && (
                <div className="worker-dynamic-grid worker-ng-grid">
                    {visibleNgOptions.filter((item) => selectedNg.includes(item.key)).map((item) => (
                        <div key={item.key} className="worker-field-block">
                            ...
                            <input id={item.key} className="worker-text-input" name={item.key} value={form[item.key]} onChange={(event) => { if (!qualityLocked) onNgValue(item.key, event.target.value); }} readOnly={qualityLocked} disabled={qualityLocked} inputMode="numeric" autoComplete="off" />
                        </div>
                    ))}
                </div>
            )}
