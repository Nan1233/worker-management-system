...
import type { FormState, MachineLineState, NgKey, OperationMode, OperationType } from "../processPageConfig";
                ...
                {!usesMultiMachineLines && !usesSingleMachine && (
                    <div className="worker-selection-card worker-field-full">
                        <div className="worker-selection-heading">
                            <div><strong>Mã sản phẩm</strong><span className="worker-required">*</span><small>Danh sách theo đúng công đoạn đang nhập</small></div>
                        ...
                        <AutocompleteInput id="productName" label="Mã sản phẩm" value={form.productName} options={productAutocompleteOptions}
                            placeholder={loadingMasterData ? "Đang tải danh mục sản phẩm…" : "Nhập hoặc chọn mã sản phẩm"} required disabled={loadingMasterData}
                            emptyMessage="Danh mục sản phẩm đang trống. Hệ thống sẽ tự tải lại dữ liệu danh mục." onChange={setProduct} onSelect={(option) => setProduct(option.value)} />
                    </div>
                )}
                            ...
                            {machineLines.map((line, index) => (
                                <article className="machine-line" key={index}>
                                    <div className="machine-card-header">
                                        <div className="machine-card-title-wrap"><span className="machine-card-number">{index + 1}</span><div><strong>Máy {index + 1}</strong><span>{line.machineCode || "Chưa chọn máy"}</span></div></div>
                                    ...
                                    <div className="machine-selection-grid">
                                        ...
                                        <AutocompleteInput id={`machineProduct-${index}`} label="Mã sản phẩm" value={line.productCode} options={getMachineProductAutocompleteOptions(line.machineCode)}
                                            placeholder={line.machineCode.trim() ? "Chọn mã sản phẩm theo máy" : "Chọn máy trước"} required disabled={loadingMasterData || !line.machineCode.trim()}
                                    ...
                                    <div className="machine-data-grid">
                                        <div><div className="machine-section-title">Thời gian chạy máy</div><div className="machine-time-row">
                                        ...
                                        <div className="machine-deduction-options">
                                            {activeNgOptions.map((item) => (
                                                <label key={item.key} className="machine-deduction-option">
                                                    <input type="checkbox" className="machine-ng-checkbox" style={{ width: 16, height: 16, minWidth: 16, maxWidth: 16, minHeight: 16, maxHeight: 16, flex: "0 0 16px", boxSizing: "border-box", margin: 0, padding: 0 }} checked={line.selectedDefects.includes(item.key)} onChange={() => toggleMachineDefect(index, item.key)} />
