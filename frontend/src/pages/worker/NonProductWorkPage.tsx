...
export default function NonProductWorkPage() {
  ...
  `}</style><div className="cvk-shell">
    ...
    <section className="cvk-card"><form className="cvk-form" onSubmit={submit}>{message && message.toLowerCase().includes("thành công") && <div className="cvk-success">✓ {message}</div>}
      ...
      <section className="cvk-section"><h2 className="cvk-section-title">Hiệu suất &amp; Thời gian</h2><div className="cvk-time"><label><span className="cvk-label">Giờ</span><input className="cvk-input" inputMode="numeric" min="0" max="12" type="number" value={hours} onChange={e => updateActualTime(e.target.value, minutes)} placeholder="0" /></label><label><span className="cvk-label">Phút</span><input className="cvk-input" inputMode="numeric" min="0" max="59" type="number" value={minutes} onChange={e => updateActualTime(hours, e.target.value)} placeholder="0" /></label><div className="cvk-total-wrap"><span className="cvk-label">Tổng thời gian</span><div className="cvk-total"><strong>{totalHours > 0 ? `${totalHours.toFixed(2)} giờ` : "Chưa nhập"}</strong></div></div></div><div className="cvk-summary"><div><small>Thực tế</small><strong>{actualHours.toFixed(2)} giờ</strong></div><div><small>Thời gian trừ</small><strong>{deductionHours.toFixed(2)} giờ</strong></div><div><small>Tổng</small><strong>{totalHours.toFixed(2)} / 12 giờ</strong></div></div>
