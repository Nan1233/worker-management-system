...
export function aggregateMachineLines(lines: MachineLineState[], activeNgOptions: NgOption[] = []) {
  ...
  const result = lines.reduce((a, l) => {
    ...
    for (const key of Array.isArray(l.selectedDefects) ? l.selectedDefects : []) selected.add(String(key));
  ...
  return { ...result, selectedNg: [...selected], firstProductCode, totalStandardOutput: totalStandardOutput || result.totalMaximum, defects };
}

export const getMachineNgTotal = (line: MachineLineState): number => Number(line.ngQuantity) || 0;

export function getMaxMachineCount(processCodeOrLines: string | MachineLineState[], linesOrMax?: MachineLineState[] | number, machineOptions?: unknown[]): number {
  const lines = Array.isArray(processCodeOrLines) ? processCodeOrLines : (Array.isArray(linesOrMax) ? linesOrMax : []);
...
export function toggleMachineDefectLine(line: MachineLineState, key: string, checked?: boolean): MachineLineState {
  ...
  if (nextChecked) selected.add(key); else { selected.delete(key); delete defects[key]; }
  const ngQuantity = Object.values(defects).reduce((sum, value) => sum + Math.max(0, Math.trunc(Number(value) || 0)), 0);
  return { ...line, selectedDefects: Array.from(selected), defects, ngQuantity: String(ngQuantity) };
}
...
export function updateMachineDefectLine(line: MachineLineState, key: string, value: string): MachineLineState {
  ...
  return { ...line, defects, ngQuantity: String(ngQuantity) };
}

// Per-machine adjustment fields are handled by the worker form.
