...
function Login() {
    ...
    useEffect(() => {
        ...
        if (passiveCrossTabRedirect) clearCurrentTabAuthSession(); else { beginLoginTransition(); clearAuthSession({ bumpEpoch: false }); finishLoginTransition(); }
    ...
    const completeLogin = async (type: AccessType) => {
        ...
        try {
            ...
            if (!user?.role) throw new Error("Đăng nhập thành công nhưng máy chủ không trả về thông tin tài khoản.");
            if (rememberAccount) localStorage.setItem(REMEMBERED_CODE_KEY, rawUsername); else localStorage.removeItem(REMEMBERED_CODE_KEY);
    ...
    return (
        <main className="login-page">
            <section className="login-card" aria-label="Đăng nhập hệ thống KTC">
                <div className="login-brand"><img src={loginLogoSrc} onError={() => { if (loginLogoSrc !== LOGIN_LOGO_FALLBACK_URL) setLoginLogoSrc(LOGIN_LOGO_FALLBACK_URL); }} alt="KTC HANOI" className="login-logo" width="192" height="192" decoding="async" /></div>
                <header className="login-heading"><h1>{step === "role-choice" ? "Chọn vai trò" : step === "management-password" ? "Đăng nhập quản lý" : "Chào mừng bạn!"}</h1><p>{step === "employee-code" && "Nhập mã nhân viên để tiếp tục"}{step === "role-choice" && <>Mã nhân viên: <strong>{username.trim()}</strong><br />Chọn loại tài khoản để tiếp tục</>}{step === "management-password" && <>Mã nhân viên: <strong>{username.trim()}</strong></>}</p></header>
                {step === "employee-code" && <form className="login-form" onSubmit={continueWithCode}>
                    <div className="login-field"><label htmlFor="login-username">Mã nhân viên</label><div className="login-input-wrap"><span className="login-input-icon" aria-hidden="true">♙</span><input data-testid="login-username" id="login-username" type="text" inputMode="text" autoComplete="username" placeholder="Nhập mã nhân viên" value={username} onChange={(event) => setUsername(event.target.value)} disabled={loading} autoFocus maxLength={20} /></div></div>
                    ...
                    {error && <div className="login-error" role="alert">{error}</div>}<button type="submit" className="login-submit" disabled={loading}>Tiếp tục <span aria-hidden="true">→</span></button>
                </form>}
                {step === "role-choice" && <div className="login-role-choice">
                    <button type="button" data-testid="login-role-worker" className="login-role-card" onClick={() => chooseRole("worker")} disabled={loading}><span className="login-role-icon" aria-hidden="true">♙</span><span className="login-role-copy"><strong>Công nhân</strong><small>Đăng nhập bằng mã nhân viên đã nhập</small></span><b aria-hidden="true">›</b></button>
                    <button type="button" data-testid="login-role-management" className="login-role-card" onClick={() => chooseRole("management")} disabled={loading}><span className="login-role-icon" aria-hidden="true">♙</span><span className="login-role-copy"><strong>Quản lý</strong><small>Dùng mã nhân viên và nhập mật khẩu</small></span><b aria-hidden="true">›</b></button>
                ...
                <footer className="login-footer"><strong>KTC (HANOI) CO., LTD</strong><span>Production Management System</span><i aria-hidden="true" /><span>v1.0.0</span></footer>
            </section>
        </main>
    );
}
