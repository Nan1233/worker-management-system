...
const initialMonthRange = () => { const today = parseDate(getToday()); return { from: isoDate(monthStart(today)), to: isoDate(monthEnd(today)) }; };
...
export default function ManagerExcelReports() {
  ...
  const applyPreset = (next: DatePreset) => {
    ...
    setPreset(next); setDateFrom(isoDate(from)); setDateTo(isoDate(to));
  };
  ...
  const load = async () => {
    try {
      ...
      setReports(rows); setTotal(status === "all" ? rows.length : result.pagination?.total || rows.length);
  ...
  const deductionColumns = useMemo(() => {
    ...
    rows.forEach(r => (r.deductions || []).forEach(d => { if (d.deduction_name) names.add(String(d.deduction_name)); }));
  ...
  const save = async () => {
    if (!draft?.id) return;
    try { setSaving(true); const source = draft.status === "pending" ? "pending" : "approved"; await updateReport(Number(draft.id), draft, source, draft.updated_at || null); showToast("Đã cập nhật báo cáo", "success"); setEditing(null); setDraft(null); await load(); }
  ...
  const columnCount = 18 + DEFECT_FIELDS.length + deductionColumns.length + 3;
