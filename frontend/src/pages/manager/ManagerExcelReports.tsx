...
const DEFECT_FIELDS: Array<[keyof ProductionReport, string]> = [
  ["kqd_dap_lai", "KQD đập lại"], ["kqd_tuot", "KQD tuột"], ["vo_do_long", "Vỡ"],
  ["xuoc_do_long", "Xước"], ["cong_gay", "Công gãy"], ["xoay", "Xoay"], ["khong_dut", "Không đứt"],
  ["bavia_hut", "Bavia hút"], ["ppcm", "PPCM"], ["loi_cao_su", "Lỗi cao su"], ["ng_kich_thuoc", "NG kích thước"], ["cat_lem", "Cắt lẹm"],
];
...
export default function ManagerExcelReports() {
  ...
  const load = async () => {
    try {
      ...
      const result = status === "approved" ? await getApprovedReports(filters) : status === "pending" ? await getPendingReports(filters) : await getPendingReports(filters);
  ...
  return <div className="manager-excel-page">
    <header className="manager-excel-head"><div><h1>Quản lý báo cáo</h1><p>Bảng dữ liệu sản xuất đầy đủ, trình bày theo kiểu Excel để xem trực tiếp.</p></div><div className="manager-excel-head-actions"><button type="button" onClick={() => void load()}>↻ Làm mới</button>{selected.length > 0 && <button className="primary" type="button" onClick={() => void approve()}>✓ Duyệt {selected.length}</button>}</div></header>
    ...
    <section className="manager-excel-filters"><label>Ngày<input type="date" value={date} onChange={e => setDate(e.target.value)} /></label><label>Trạng thái<select value={status} onChange={e => setStatus(e.target.value as typeof status)}><option value="all">Tất cả</option><option value="pending">Chờ duyệt</option><option value="approved">Đã duyệt</option></select></label><label className="search">Tìm kiếm<input value={search} onChange={e => setSearch(e.target.value)} placeholder="Mã CN, họ tên, máy, sản phẩm..." /></label></section>
    <section className="manager-excel-table-card"><div className="manager-excel-table-wrap"><table className="manager-excel-table"><thead><tr><th className="sticky select"><input type="checkbox" checked={rows.length > 0 && rows.every(r => selected.includes(Number(r.id)))} onChange={e => setSelected(e.target.checked ? rows.map(r => Number(r.id)).filter(Boolean) : [])} /></th><th className="sticky date">Ngày</th><th className="sticky code">Mã CN</th><th className="sticky name">Họ tên</th><th>Công đoạn</th><th>Ca</th><th>Máy</th><th>Sản phẩm</th><th>% HV</th><th>Tổng giờ</th><th>Trừ giờ</th><th>TT giờ</th><th>Định mức</th><th>Thực tế</th><th>OK</th><th>NG</th><th>% đạt</th><th>% NG</th>{DEFECT_FIELDS.map(([, label]) => <th key={label}>{label}</th>)}<th>Trừ giờ chi tiết</th><th>Lỗi chi tiết</th><th>Ghi chú</th><th>Trạng thái</th><th className="action">Thao tác</th></tr></thead><tbody>{loading ? <tr><td colSpan={28} className="empty">Đang tải dữ liệu...</td></tr> : !rows.length ? <tr><td colSpan={28} className="empty">Không có báo cáo phù hợp.</td></tr> : rows.map((r, i) => {
      ...
      return <tr key={`${id}-${i}`} className={`${selected.includes(id) ? "selected" : ""} ${isEdit ? "editing" : ""}`} onClick={() => { if (!isEdit) setSelected(v => v.includes(id) ? v : [...v, id]); }}>
        ...
        <td>{text(d.process_name)}</td><td>{text(d.shift)}</td><td>{text(d.machine_no)}</td><td className="wide">{text(d.product_name)}</td><td>{fmt(d.training_percent ?? (d as any).training_percent_snapshot)}%</td>
        <td>{fmt(d.total_time)}</td><td>{fmt(d.deduction_time)}</td><td>{fmt(d.actual_time)}</td><td>{fmt(d.target_output ?? d.standard_output)}</td><td>{isEdit ? <input className="cell-input" type="number" value={n(d.actual_output)} onChange={e => setField("actual_output", Number(e.target.value))} /> : qty(d.actual_output)}</td><td>{isEdit ? <input className="cell-input" type="number" value={n(d.tt_ok)} onChange={e => setField("tt_ok", Number(e.target.value))} /> : qty(d.tt_ok)}</td><td>{qty(d.tt_ng)}</td><td>{fmt(rate(d))}%</td><td>{fmt(ngRate(d))}%</td>
        {DEFECT_FIELDS.map(([field, label]) => <td key={label}>{isEdit ? <input className="cell-input defect" type="number" value={n(d[field])} onChange={e => setField(field, Number(e.target.value))} /> : qty(d[field])}</td>)}
        <td className="detail-cell">{detailsRow.length ? detailsRow.map(x => `${text(x.deduction_name)} ${fmt(n(x.hours) * 60)}p`).join(" · ") : "—"}</td><td className="detail-cell">{defects.length ? defects.map(x => `${text(x.defect_name)}: ${qty(x.quantity)}`).join(" · ") : "—"}</td><td className="wide">{text(d.note)}</td><td><span className={`status ${d.status || "pending"}`}>{d.status === "approved" ? "Đã duyệt" : d.status === "rejected" ? "Từ chối" : "Chờ duyệt"}</span></td>
        <td className="action" onClick={e => e.stopPropagation()}>{isEdit ? <div className="row-actions"><button type="button" onClick={() => void save()} disabled={saving}>Lưu</button><button type="button" onClick={() => { setEditing(null); setDraft(null); }}>Hủy</button></div> : <div className="row-actions"><button type="button" onClick={() => startEdit(r)}>Sửa</button>{d.status === "approved" && <button type="button" className="danger" onClick={() => void remove(r)}>Xóa</button>}</div>}</td>
      </tr>;
