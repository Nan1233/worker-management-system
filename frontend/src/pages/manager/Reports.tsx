...
function Reports() {
    ...
    const startInlineEdit = () => { if (!selectedDetail || !canDirectEdit) return; const actual = Math.max(0, Number(selectedDetail.actual_time) || 0); const hours = Math.floor(actual); const minutes = Math.round((actual - hours) * 60); setEditDraft({ ...selectedDetail }); setEditHours(String(hours)); setEditMinutes(String(Math.min(59, minutes))); setEditingDetail(true); };
    ...
    const saveInlineEdit = async () => { if (!editDraft || !editDraft.id || editSaving) return; const actualHours = Math.max(0, Number(editHours) || 0); const actualMinutes = Math.min(59, Math.max(0, Number(editMinutes) || 0)); const actualTime = actualHours + actualMinutes / 60; const deductionTime = Number(editDraft.deduction_time) || 0; const ok = Math.max(0, Number(editDraft.tt_ok) || 0); const ng = Math.max(0, Number(editDraft.tt_ng) || 0); const nextDraft: ProductionReport = { ...editDraft, work_date: String(editDraft.work_date || "").slice(0, 10), actual_time: actualTime, total_time: actualTime + deductionTime, tt_ok: ok, tt_ng: ng, actual_output: ok + ng }; try { setEditSaving(true); const result = await updateReport(Number(editDraft.id), nextDraft, "pending"); const updated = result?.data || result?.report || result; const merged = { ...nextDraft, ...(updated && typeof updated === "object" ? updated : {}) } as ProductionReport; setSelectedDetail(merged); setReports(current => current.map(item => Number(item.id) === Number(merged.id) ? { ...item, ...merged } : item)); setEditDraft(null); setEditingDetail(false); showToast("Đã cập nhật báo cáo", "success"); await loadReports(); } catch (err: unknown) { showToast(axios.isAxiosError(err) ? err.response?.data?.message || "Không thể cập nhật báo cáo" : "Không thể cập nhật báo cáo"); } finally { setEditSaving(false); } };
    const openProposal = () => { if (!selectedDetail || !isLead) return; setProposalReason(""); setProposalOpen(true); };
    const sendProposal = async () => { if (!selectedDetail || proposalSending) return; const reason = proposalReason.trim(); if (reason.length < 2) return showToast("Vui lòng nhập nội dung đề xuất sửa"); try { setProposalSending(true); await api.post(`/production-temp/${Number(selectedDetail.id)}/request-edit`, { reason }); showToast("Đã gửi đề xuất sửa cho công nhân", "success"); setProposalOpen(false); setProposalReason(""); } catch (err: unknown) { showToast(axios.isAxiosError(err) ? err.response?.data?.message || "Không thể gửi đề xuất sửa" : "Không thể gửi đề xuất sửa"); } finally { setProposalSending(false); } };
            ...
            <section className={`pending-workspace ${selectedDetail ? "detail-open" : "list-only"}`}>
                ...
                {selectedDetail && <aside className={`pending-detail-card ${editingDetail ? "is-editing" : ""}`}>
                    ...
                    {detailLoading ? <div className="pending-detail-loading">Đang tải chi tiết...</div> : detail ? <>
                        ...
                        <footer className="pending-detail-actions" style={{ gridTemplateColumns: editingDetail ? "1fr 1fr" : isLead ? "1.35fr 1fr 1fr" : "1fr 1fr 1fr" }}>
