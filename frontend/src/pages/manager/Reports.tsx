...
import { approveSelectedTempReports, getPendingReports, getTempReportDetail, rejectSelectedTempReports, updateReport } from "../../services/productionService";
...
const timeRange = (report: ProductionReport) => { const extra = report.extra_data || {}; const start = extra.start_time; const end = extra.end_time; return start && end ? `${String(start)} - ${String(end)}` : "07:30 - 15:30"; };
...
const rangeFor = (value: string, type: "year" | "month" | "week" | "day") => { const base = toDate(value); const start = new Date(base); const end = new Date(base); if (type === "year") { start.setMonth(0, 1); end.setMonth(11, 31); } else if (type === "month") { start.setDate(1); end.setMonth(end.getMonth() + 1, 0); } else if (type === "week") { const mondayOffset = (start.getDay() + 6) % 7; start.setDate(start.getDate() - mondayOffset); end.setTime(start.getTime()); end.setDate(start.getDate() + 6); } return { dateFrom: dateString(start), dateTo: dateString(end) }; };
...
function Reports() {
    ...
    const loadReports = useCallback(async () => {
        ...
        try {
            ...
            const [yearResult, monthResult, weekResult, dayResult] = await Promise.all([
                getPendingReports({ ...ranges.year, ...filters, page: 1, pageSize: 1 }), getPendingReports({ ...ranges.month, ...filters, page: 1, pageSize: 1 }),
                getPendingReports({ ...ranges.week, ...filters, page: 1, pageSize: 1 }), getPendingReports({ ...ranges.day, ...filters, page: 1, pageSize: 1 }),
            ]);
            if (request !== seq.current) return;
    ...
    const openDetail = async (report: ProductionReport) => { const id = Number(report.id); if (!id) return; setSelectedDetail(report); setEditDraft(null); setEditingDetail(false); setDetailLoading(true); try { setSelectedDetail(await getTempReportDetail(id)); } catch (err) { showToast(axios.isAxiosError(err) ? err.response?.data?.message || "Không thể tải chi tiết báo cáo" : "Không thể tải chi tiết báo cáo"); } finally { setDetailLoading(false); } };
    const prepareEditDraft = (report: ProductionReport) => { const actual = Math.max(0, Number(report.actual_time) || 0); const hours = Math.floor(actual); const minutes = Math.round((actual - hours) * 60); setEditDraft({ ...report, defects: [...(report.defects || [])], deductions: [...(report.deductions || [])] }); setEditHours(String(hours)); setEditMinutes(String(Math.min(59, minutes))); };
    const startInlineEdit = () => { if (!selectedDetail || !canDirectEdit) return; prepareEditDraft(selectedDetail); setEditingDetail(true); };
    ...
    const updateDefect = (index: number, quantity: number) => setEditDraft(current => { if (!current) return current; const defects = [...(current.defects || [])]; if (!defects[index]) return current; defects[index] = { ...defects[index], quantity: Math.max(0, quantity) }; const ttNg = defects.reduce((sum, item) => sum + Math.max(0, Number(item.quantity) || 0), 0); return { ...current, defects, tt_ng: ttNg, actual_output: Math.max(0, Number(current.tt_ok) || 0) + ttNg }; });
    const updateDeduction = (index: number, minutes: number) => setEditDraft(current => { if (!current) return current; const deductions = [...(current.deductions || [])]; if (!deductions[index]) return current; deductions[index] = { ...deductions[index], hours: Math.max(0, minutes) / 60 }; const deductionTime = deductions.reduce((sum, item) => sum + Math.max(0, Number(item.hours) || 0), 0); const actualTime = Math.max(0, Number(editHours) || 0) + Math.min(59, Math.max(0, Number(editMinutes) || 0)) / 60; return { ...current, deductions, deduction_time: deductionTime, actual_time: actualTime, total_time: actualTime + deductionTime }; });
    const buildEditPayload = () => {
        ...
        const defects = (editDraft.defects || []).map(item => ({ ...item, quantity: Math.max(0, Number(item.quantity) || 0) }));
        const deductions = (editDraft.deductions || []).map(item => ({ ...item, hours: Math.max(0, Number(item.hours) || 0) }));
    ...
    const saveInlineEdit = async () => { if (!editDraft || !editDraft.id || editSaving) return; const nextDraft = buildEditPayload(); if (!nextDraft) return; try { setEditSaving(true); const result = await updateReport(Number(editDraft.id), nextDraft, "pending"); const updated = result?.data || result?.report || result; const merged = { ...nextDraft, ...(updated && typeof updated === "object" ? updated : {}) } as ProductionReport; setSelectedDetail(merged); setReports(current => current.map(item => Number(item.id) === Number(merged.id) ? { ...item, ...merged } : item)); setEditDraft(null); setEditingDetail(false); showToast("Đã cập nhật báo cáo", "success"); await loadReports(); } catch (err: unknown) { showToast(axios.isAxiosError(err) ? err.response?.data?.message || "Không thể cập nhật báo cáo" : "Không thể cập nhật báo cáo"); } finally { setEditSaving(false); } };
    const openProposal = () => { if (!selectedDetail || !isLead) return; prepareEditDraft(selectedDetail); setEditingDetail(true); };
    const saveProposal = async (approveAfterSave = false) => {
        if (!editDraft || !editDraft.id || editSaving) return;
        const nextDraft = buildEditPayload();
        if (!nextDraft) return;
        try {
            setEditSaving(true);
            const reason = String(nextDraft.note || "").trim() || "Tổ trưởng chỉnh sửa báo cáo";
            const result = await api.put(`/production-temp/${Number(editDraft.id)}`, { ...nextDraft, reason });
            ...
            showToast(approveAfterSave ? "Đã sửa báo cáo và chuyển sang đã duyệt" : "Đã sửa báo cáo, báo cáo vẫn chờ duyệt", "success");
        ...
        } catch (err: unknown) { showToast(axios.isAxiosError(err) ? err.response?.data?.message || "Không thể sửa báo cáo" : "Không thể sửa báo cáo"); } finally { setEditSaving(false); }
    };
    ...
    const editableDefects = detail?.defects || [];
    const editableDeductions = detail?.deductions || [];
    const detailTotal = Number(detail?.actual_output || 0); const detailOk = Number(detail?.tt_ok || 0); const detailNg = Number(detail?.tt_ng || 0); const detailRate = detailTotal > 0 ? (detailOk / detailTotal) * 100 : 0;
    const selectDay = () => { setDate(getToday()); setDateRange(null); }; const selectMonth = () => setDateRange(rangeFor(date, "month")); const selectWeek = () => setDateRange(rangeFor(date, "week")); const selectYear = () => setDateRange(rangeFor(date, "year"));
    ...
    return (
        <div className="management-report-page manager-page pending-reference-page">
            ...
            <section className="pending-filter-card"><div className="pending-search"><span>⌕</span><input value={searchKeyword} onChange={event => setSearchKeyword(event.target.value)} placeholder="Tìm kiếm mã báo cáo, công nhân..." /></div><label><span>Ngày báo cáo</span><input type="date" value={date} onChange={event => { setDate(event.target.value); setDateRange(null); }} /></label><div className="pending-quick-filters"><span>Chọn nhanh</span><button type="button" className={rangeIsActive("day") ? "active" : ""} onClick={selectDay}>Hôm nay</button><button type="button" className={rangeIsActive("week") ? "active" : ""} onClick={selectWeek}>Tuần này</button><button type="button" className={rangeIsActive("month") ? "active" : ""} onClick={selectMonth}>Tháng này</button><button type="button" className={rangeIsActive("year") ? "active" : ""} onClick={selectYear}>Năm này</button></div><label><span>Công đoạn</span><select value={selectedProcess} onChange={event => setSelectedProcess(event.target.value)}><option value="">Tất cả</option>{processes.map(process => <option key={process} value={process}>{process}</option>)}</select></label><label><span>Ca làm việc</span><select value={selectedShift} onChange={event => setSelectedShift(event.target.value)}><option value="">Tất cả</option>{shifts.map(shift => <option key={shift}>{shift}</option>)}</select></label><button className="pending-refresh" type="button" onClick={() => void loadReports()}>⟳ <span>Làm mới</span></button></section>
                    ...
                    {detailLoading ? <div className="pending-detail-loading">Đang tải chi tiết...</div> : detail ? <>
                        <div className="pending-detail-body">
                            ...
                            <section className="pending-detail-section"><h3>Thông tin chi tiết</h3>{editingDetail ? <div className="pending-detail-edit-details">
                                <div className="pending-detail-edit-block"><div className="pending-detail-edit-heading"><strong>Chi tiết thời gian trừ</strong><span>{number((detail.deductions || []).reduce((sum, item) => sum + Number(item.hours || 0), 0) * 60)} phút</span></div>{editableDeductions.length ? <div className="pending-edit-grid">{editableDeductions.map((item, index) => <label key={item.id || item.deduction_code || index}><span>{item.deduction_name || item.deduction_code || "Thời gian trừ"}</span><div className="pending-edit-number-with-unit"><input type="number" min="0" max="1440" step="1" value={Math.round(Number(item.hours || 0) * 60)} onChange={e => updateDeduction(index, Number(e.target.value) || 0)} /><small>phút</small></div></label>)}</div> : <p className="pending-history-empty">Không có chi tiết thời gian trừ.</p>}</div>
                                <div className="pending-detail-edit-block"><div className="pending-detail-edit-heading"><strong>Chi tiết lỗi NG</strong><span>{number((detail.defects || []).reduce((sum, item) => sum + Number(item.quantity || 0), 0))}</span></div>{editableDefects.length ? <div className="pending-edit-grid">{editableDefects.map((item, index) => <label key={item.id || item.defect_code || index}><span>{item.defect_name || item.defect_code || "Lỗi NG"}</span><input type="number" min="0" step="1" value={Number(item.quantity || 0)} onChange={e => updateDefect(index, Number(e.target.value) || 0)} /></label>)}</div> : <p className="pending-history-empty">Không có chi tiết lỗi NG.</p>}</div>
                        ...
                        <footer className="pending-detail-actions" style={{ gridTemplateColumns: editingDetail ? (isLead ? "1fr 1.15fr 1.15fr" : "1fr 1fr") : isLead ? "1.35fr 1fr 1fr" : "1fr 1fr 1fr" }}>
                            {editingDetail ? isLead ? <><button type="button" className="pending-detail-cancel" onClick={cancelInlineEdit} disabled={editSaving}>Hủy</button><button type="button" className="pending-detail-save" onClick={() => void saveProposal(false)} disabled={editSaving}>{editSaving ? "Đang lưu..." : "Lưu, chờ duyệt"}</button><button type="button" className="pending-detail-approve" onClick={() => void saveProposal(true)} disabled={editSaving}>{editSaving ? "Đang xử lý..." : "Lưu & duyệt"}</button></> : <><button type="button" className="pending-detail-cancel" onClick={cancelInlineEdit} disabled={editSaving}>Hủy</button><button type="button" className="pending-detail-save" onClick={() => void saveInlineEdit()} disabled={editSaving}>{editSaving ? "Đang lưu..." : "Lưu thay đổi"}</button></> : <>
                                {isLead && <button type="button" className="pending-detail-edit" onClick={openProposal} disabled={editSaving}>Đề xuất sửa</button>}
