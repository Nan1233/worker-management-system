...
export default function SystemCenter(){
 ...
 const load=async()=>{setLoading(true);try{const [n,a]=await Promise.all([getNotifications(),getActivities()]);setNotifications(n.data);setActivities(a);}finally{setLoading(false)}};
 useEffect(()=>{void load()},[]);
