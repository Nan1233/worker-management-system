...
export default function FormulaSettings(){
 ...
 const [data,setData]=useState<FormulaResponse>({products:[],scopes:[]});
 const [selectedScope,setSelectedScope]=useState('GLOBAL');
 const [draft,setDraft]=useState<FormulaScope|null>(null);
 const [loading,setLoading]=useState(true); const [saving,setSaving]=useState(false); const [savingId,setSavingId]=useState<number|null>(null);
 const [error,setError]=useState(''); const [success,setSuccess]=useState(''); const [query,setQuery]=useState(''); const [productProcessId,setProductProcessId]=useState('');
 ...
 const save=async()=>{if(!draft)return;setSaving(true);setError('');setSuccess('');try{const payload={...draft,output_formula:draft.output_formula.trim(),output_per_hour_formula:draft.output_per_hour_formula.trim(),achievement_formula:draft.achievement_formula.trim(),ng_rate_formula:draft.ng_rate_formula.trim(),actual_time_formula:draft.actual_time_formula.trim()};const response=await api.put(`/formula-settings/scopes/${encodeURIComponent(draft.scope_code)}`,payload);const next=response.data.data as {scopes:FormulaScope[]};setData(prev=>({...prev,scopes:next.scopes}));const scope=next.scopes.find(item=>item.scope_code===draft.scope_code)||draft;setDraft({...scope});setSuccess('Đã lưu công thức và ngưỡng. Công thức được kiểm tra trước khi hệ thống sử dụng.');}catch(e){setError(getApiError(e,'Không thể lưu công thức').message);}finally{setSaving(false);}};
