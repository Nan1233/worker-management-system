...
export default function Permissions(){
 ...
 if(loading) return <div className="permission-page"><div className="permission-loading">Đang tải cấu hình quyền...</div></div>;
 ...
 return <section className="permission-page">
  ...
  <div className="permission-card"><div className="permission-card-head"><div><h2>Quyền mặc định theo vai trò</h2><p>Manager, Tổ trưởng và Công nhân có thể được điều chỉnh; ô “Mặc định” trả về cấu hình chuẩn của hệ thống.</p></div></div>
   <div className="permission-table-wrap"><table><thead><tr><th>Chức năng</th><th>Manager</th><th>Tổ trưởng</th><th>Công nhân</th></tr></thead><tbody>
    {modules.map(([module,items])=><Fragment key={module}><tr className="module-row"><td colSpan={4}>{module.toUpperCase()}</td></tr>{items.map(p=><tr key={p.code}><td><strong>{p.name}</strong><small>{p.code}</small></td>{['manager','lead','worker'].map(role=>{const row=data.roles.find(r=>r.role===role)!; const value=Object.prototype.hasOwnProperty.call(row.overrides,p.code)?row.overrides[p.code]:row.defaults[p.code]; return <td key={role}>{row.capabilities[p.code] ? <select value={Object.prototype.hasOwnProperty.call(row.overrides,p.code)?String(value):'default'} onChange={e=>void setRole(role,p.code,e.target.value==='default'?null:e.target.value==='true')}><option value="default">Mặc định ({row.defaults[p.code]?'Cho phép':'Từ chối'})</option><option value="true">Cho phép</option><option value="false">Từ chối</option></select> : <span className="permission-na">Không áp dụng</span>}</td>})}</tr>)}</Fragment>)}
   </tbody></table></div>
  </div>
  <div className="permission-card"><div className="permission-card-head permission-user-head"><div><h2>Quyền riêng từng tài khoản</h2><p>Override riêng được áp dụng sau quyền vai trò và chỉ nên dùng cho ngoại lệ.</p></div><select value={userId} onChange={e=>setUserId(e.target.value)}><option value="">Chọn tài khoản...</option>{data.users.filter(u=>u.role!=='admin').map(u=><option key={u.id} value={u.id}>{u.full_name||u.username} · {roleNames[u.role]||u.role}</option>)}</select></div>
   {selected ? <div className="permission-user-grid">{data.permissions.map(p=>{const roleRow=data.roles.find(r=>r.role===selected.role); const supported=Boolean(roleRow?.capabilities[p.code]); return <label key={p.code} className={!supported?'is-disabled':''}><span><strong>{p.name}</strong><small>{p.code}</small></span><select disabled={!supported} value={Object.prototype.hasOwnProperty.call(uo,p.code)?String(uo[p.code]):'default'} onChange={e=>void setUser(p.code,e.target.value==='default'?null:e.target.value==='true')}><option value="default">Theo vai trò</option><option value="true">Cho phép</option><option value="false">Từ chối</option></select></label>})}</div> : <div className="permission-empty">Chọn một tài khoản để cấu hình ngoại lệ.</div>}
  </div>
 </section>
}
