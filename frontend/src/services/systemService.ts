...
export async function getUnreadNotificationCount(){ const r=await api.get('/system/notifications/unread-count'); return Number(r.data?.data?.unreadCount ?? r.data?.unread ?? 0); }
