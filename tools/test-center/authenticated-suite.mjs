...
async function createWorker(base, managerToken, processIds) {
  ...
  const r = await api(base, '/api/users', {
    ...
    body: JSON.stringify({
      username, full_name: fullName, role: 'worker', worker_code: workerCode,
      department: 'TEST ONLY', position: 'Công nhân', training_percent: 100,
      status: 'active', process_ids: processIds,
    }),
  });
...
async function createReport(base, workerToken, ctx, values = {}) {
  ...
  const payload = {
    process_id: ctx.process.id, process_code: ctx.process.code, work_date: new Date().toISOString().slice(0,10),
    shift: values.shift || `TC${Math.floor(Math.random()*3)+1}`,
    machine_no: ctx.machineCode || undefined, product_name: ctx.productCode,
    operation_mode: ctx.machineCode ? 'MACHINE' : 'MANUAL', operation_type: 'PRODUCTION',
    total_time: total, actual_time: actual, deduction_time: deduction,
    standard_output: ctx.standardOutput, actual_output: ok, tt_ok: ok, tt_ng: ng,
    defects: values.defects || [], deductions: values.deductions || [],
    training_percent: 100, client_request_id: unique('tc-report'),
...
export async function runAuthenticatedSuite({ apiUrl, add, managerUsername = '', managerPassword = '' }) {
  if (!managerUsername || !managerPassword) {
    for (const [id,name,detail] of [
      ['MASTER-001','Cắt has CAT01-CAT04','Thiếu KTC_TEST_MANAGER_USERNAME/PASSWORD'],
      ['MASTER-002','Lồng has LONG01-LONG05','Thiếu KTC_TEST_MANAGER_USERNAME/PASSWORD'],
      ['WORKER-001','Create production report','Thiếu manager fixture'],
      ['WORKER-002','View report history','Thiếu worker fixture'],
      ['WORKER-003','Edit report within 10 minutes','Thiếu worker fixture'],
      ['RULE-001','Daily total <= 12 hours','Thiếu worker fixture'],
      ['RULE-002','Support/deduction excluded from daily production hours','Thiếu worker fixture'],
      ['MANAGER-001','Manager review/approve','Thiếu manager fixture'],
      ['PERM-001','Role authorization','Thiếu role fixture'],
      ['EXPORT-001','Excel export','Thiếu manager fixture'],
  ...
  // 12h ceiling: two 6h reports must be accepted, then an additional report must be rejected.
  ...
  // Daily-hours must count actual_time, not deduction/support time.
  ...
  // Manager can see and approve the newly created worker report.
