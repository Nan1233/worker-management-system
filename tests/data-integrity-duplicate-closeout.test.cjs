...
test('approved duplicate prompt disables edit-existing worker action', () => {
  const page = read('frontend/src/pages/worker/ProcessPage.tsx');
  const actions = read('frontend/src/pages/worker/components/ProcessSubmitActions.tsx');
...
test('duplicate classification reads the token from the Axios response payload', () => {
  const outcome = read('frontend/src/utils/processSubmitOutcome.ts');
  const logic = read('frontend/src/pages/worker/processDuplicateReportLogic.ts');
