...
export async function loadMyPermissions(force=false): Promise<Set<PermissionCode>> {
  ...
  if(!force && cache && cache.userId===Number(user.id) && cache.expiresAt>Date.now()) return new Set(cache.values);
  try {
    const response=await api.get('/permissions/me');
    const values=new Set<PermissionCode>((response.data?.data?.permissions || []) as PermissionCode[]);
    cache={userId:Number(user.id),values,expiresAt:Date.now()+60_000}; return new Set(values);
